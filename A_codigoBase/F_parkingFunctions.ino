...
float scannerLlubot(float ancho = 20){
  ...
  for (int i=minDegFront; i<=maxDegFront; i+=10) { 
    ...
    readCm = medianFilter.GetFiltered();
    Serial.print("El resultado del filtro de medianas es: ");
    Serial.println(readCm);

    if (readCm < 40){ // Si la distancia es mayor a 50cm, no se tiene en cuenta debido a la curva de sensibilidad del sensor
